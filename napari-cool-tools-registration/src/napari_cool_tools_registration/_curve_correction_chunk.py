...
def spherical2cartesian_chunked(r, thx, thy, grid, x, y, z, order=1, chunk_size=64):
    ...
    for z_start in tqdm(range(0, len(z), chunk_size), desc="Processing", unit="chunk"):
        ...
        valid_mask = (new_ir >= 0) & (new_ir < len(r)) & \
                     (new_ithx >= 0) & (new_ithx < len(thx)) & \
                     (new_ithy >= 0) & (new_ithy < len(thy))
        ...
        if valid_mask.any():
            ...
            interpolated_chunk = map_coordinates(grid_slice, valid_points, mode='constant', cval=0)
            
            # Insert interpolated values back into the Cartesian output
            interpolated_chunk_full = cp.zeros(new_r.size, dtype=grid.dtype)  # Flat array for all points
            interpolated_chunk_full[valid_mask.ravel()] = interpolated_chunk
            output[:, :, z_start:z_end] = interpolated_chunk_full.reshape(new_r.shape).get()
...
def _on_init(widget):
    ...
    def do_curve_correct_button():
        ...
        cartify_function(input_image,widget.scan_angle.value, widget.ds.value, widget.res.value, widget.threshold.value, widget.chunk_size.value, widget.circleCrop.value)
    ...
    def do_vis_3d_button():
        ...
        volumerender_function(input_image,widget.vol_threshold.value, widget.vol_opac.value)
...
@magic_factory(
    ...
    ds=dict(widget_type="FloatSpinBox", value=1),
...
def cartify(input_vol: Image, scan_angle, ref_indx, imaging_range, pivot_point, ref_motor_loc, img_motor_loc, 
            ds, res, threshold, chunk_size, circleCrop, curve_correct_button, 
...
def cartify_function(input_image: Image, sweep, ds, res, threshold, chunk_size, circleCrop):
    ...
    cart_image = spherical2cartesian_chunked(r, thx, thy, data, x, y, z, sweep, chunk_size=chunk_size)
