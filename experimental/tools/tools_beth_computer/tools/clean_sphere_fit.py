...
def main():
    ...
    display_in_napari = True #False True
    ...
    for retchor_path in retchor_progress_bar:
        ...
        if xml_path.exists():
            ...
            pixel_spacing = imaging_range_in_water / retina_data.shape[1]
            if verbose:
                print(f"imaging range in water / A-scan pixels = pixel spacing: {imaging_range_in_water} / {retina_data.shape[1]} = {pixel_spacing}\n")
            ...
            if retina_data.sum() > 0:
                (
                    thickness_map, retina_points, rpe_points, curv_ret_points, curv_rpe_points, raw_pixel_thickness_map, pixel_thickness_map
                ) = sphere_fit_thick_map_corrected_v2(retina_data,pixel_spacing=pixel_spacing,padding_pixel=padding_pixel,refractive_index=cc_settings.refractive_index)
            ...
            thickness_threshold = 300 #250 #350 400
            ...
            slow_shape,axial_shape,fast_shape = retina_data.shape
            ...
            if perform_calculations:
                ...
                micron_error_map = (np.abs(raw_micron_thickness_map-pixel_thickness_map)/pixel_thickness_map).astype("float32")
                ...
                if save_plots or show_plots:
                    ...
                    output_plot_dir = Path(r"E:\_Beth_Thickness_Calculations\Temp_Thickness_Output")
                if save_plots:
    ...
    if len(radius_list) > 0:
        ...
        output_dataframe_dir = Path(r"E:\_Beth_Thickness_Calculations\Temp_Thickness_Output")
        output_dataframe_filepath = output_dataframe_dir/"retina_radii.csv"
        ...
        radii_ndarray = np.array(radius_list)
