...
def equidistant_motor_loss(params:np.ndarray, label_data:np.ndarray,curve_params: CurvCorrectSettings, curve_output:dict={}):#points_3D:np.ndarray):
    """
    """
    imaging_motor_position = params[0]
    center_point_3D = params[1:]

    imaging_range = curve_params.imaging_range / curve_params.refractive_index
    pixel_spacing = imaging_range / label_data.shape[1]
    
    reference_arm_shift = imaging_motor_position - curve_params.reference_motor_position # doing final position - initial position allows you to add this component
    reference_arm_shift = (reference_arm_shift * 0.5) / curve_params.refractive_index

    padding = (curve_params.pivot_point - imaging_range) + reference_arm_shift # The imaging range and pivot point are considered constant
    ...
    (rpe_points, curv_ret_points, curv_rpe_points, thickness_map) = generate_sphere_fit(
        ...
        pivot_point=curve_params.pivot_point,
        scan_angle=curve_params.scan_angle,
    )

    curve_output["points"] = curv_rpe_points
    curve_output["thickness"] = thickness_map
    curve_output['pixle_spacing'] = pixel_spacing
    ...
    # center_guess_axial = max_axial - ((15.1/2.0) / pixel_spacing) # TODO map this guess to formula for axial lengths
    # center_point_guess = np.array([center_guess_slow,center_guess_axial,center_guess_fast])
    # center_point_3D = center_point_guess

    # calculate distances from center to the sample point distribution
    distances = jnp.sqrt(jnp.sum((curv_rpe_points.astype("float64") - center_point_3D.astype("float64"))**2,axis=1))
    #return jnp.std(distances) # return standard deviation of distances for minimization
    return jnp.var(distances) # return standard deviation of distances for minimization
...
if __name__ == "__main__":
    ...
    data_path = Path(r"E:\_ret_chor_reprocessing\processed2\UNPs_08977490-2024_05_15-14_53_55_processed_ret_chor_seg.npz")
    xml_path = Path(r"E:\Beth_Thickness_Calculations\All xmls\08977490-2024_05_15-14_53_55.xml")
    ...
    fovea_point = (460,456) # pretend you got this from a file

    # create curve correction parameter class instance
    curve_params = CurvCorrectSettings(
        fovea_point=fovea_point,
        pivot_point=19.2,
        reference_motor_position=85.,
        imaging_motor_position=imaging_motor_position,
        refractive_index=1.33,
        scan_angle=100.,
    )

    center_point_guess = np.array([0.0,15.1/2.,0.0])
    ...
    params_to_fit = np.array([imaging_motor_position,0.0,15.1/2.,0.0])

    curve_output = {}

    start_time = time.perf_counter()
    print(f"start time for bfgs: {start_time}\n")

    output = fmin_bfgs(
        f=equidistant_motor_loss,
        x0=params_to_fit,
        fprime=grad(equidistant_motor_loss),
        norm=2.0,
        args=(retina_data.astype("float64"),curve_params,curve_output),
        #gtol=1e-17,
        maxiter=None,
        full_output=True,
        disp=True,
    ...
    print(f"bfgs took: {elapsed_time / 20} min\n")

    print(f"output: {output}\n")
    new_imaging_motor_position = output[0][0]
    new_center_calc = output[0][1:]
    print(f"new center: {new_center_calc} ({new_center_calc.shape}), std from new center: {jnp.sqrt(output[1])} um\n")
    print(f"curve_output: {curve_output['points'].shape,curve_output['thickness'].shape}\n")
    radius_calc = np.sqrt(np.sum((curve_output["points"] - new_center_calc)**2,axis=1)).mean()
    print(f"radius calc: {radius_calc} pixels, {radius_calc*curve_output['pixle_spacing']} um\n")
    ...
    # print(f"reference arem shift: {reference_arm_shift}\n")

    # padding = (pivot_point - imaging_range) + reference_arm_shift # The imaging range and pivot point are considered constant
    ...
    # height_map = generate_height_map_from_3D_points(rpe_points.astype(int),(label_data.shape[0],label_data.shape[2]))
    ...
    # curv_fovea_point = spherical_to_cartesian(fovea_point[None,:],label_data.shape,scan_angle=scan_angle,padding_pixel=padding_pixel)
    ...
    # print(ppp_axial)
    # projected_pivot_point = np.array([0.,ppp_axial,0.])
    ...
    #     norm=2.0,
    #     args=(curv_rpe_points.astype("float64"),),
    #     #gtol=1e-17,
    #     maxiter=None,
    #     full_output=True,
    #     disp=True,
    ...
    # radius_guess_proj = np.sqrt(np.sum((projected_pivot_point-new_center_guess)**2))
