...
def unpack12(buf: np.ndarray) -> np.ndarray:
    ...
    out = np.empty(n_triplets * 2, dtype=np.uint16)
...
def unpack12_torch(buf: torch.Tensor) -> torch.Tensor:
    ...
    return out.to(dtype=torch.float64)  # or torch.uint16 if unsigned
...
def process_unp(unp_file_path:Path):
    ...
    pause_index = [236, 265 , 296 , 327, 358]#this will make flipping problem
    # pause_index = [234, 265 , 294 , 325, 356]
    ...
    data_size_bytes = int(1.5 * w_param * h)#packed=true
    ...
    delay = (29*2)*2 + subdelay*2
    
    d = d - int(hires_h/h)*hires_d*5 #len(pause_index)=5
    
    hires_data_size_bytes = int(1.5 * w_param *  hires_h)#packed=true
        ...
        for frame_num in tqdm(range(0, d), desc="Processing Bscans"):
            ...
            if frame_num in pause_index:
                ...
                for hires_frame_num in range(0,hires_d):
                    ...
                    # raw_data = unpack_uint12_packed(byte_reader.read(hires_data_size_bytes))
                    raw_data = np.frombuffer(byte_reader.read(hires_data_size_bytes), dtype="<u1")
                    # raw_data = unpack12(raw_data)
                    # raw = raw_data.reshape((hires_h, w_param)).astype(np.float64)
                    # raw = torch.tensor(raw).cuda()
                    
                    raw_data = torch.tensor(raw_data).cuda()
                    raw = unpack12_torch(raw_data)
                    raw = raw.reshape((hires_h, w_param))
                ...
                # raw_data = unpack_uint12_packed(byte_reader.read(data_size_bytes))
...
def main():
    ...
    if path:
        ...
        fname = path[:-4] + "_processed_enface_ascancorrected_max_log.tif"
        tiff.imwrite(fname,20*np.log10(enface))
        ...
        for i in range(0,5):
            ...
            # hires_bscan = shift2d_zeropad(hires_bscan,shift=(55*2, 0))
        ...
        #     save_vol_hires[idx] = 20*np.log10(save_vol_hires[idx])
            
            if idx % 2 == 1:
                save_vol_hires[idx,:,:] = save_vol_hires[idx,::-1,:]
                
        #     save_vol_hires[idx] = desine(save_vol_hires[idx],axis = 1)
