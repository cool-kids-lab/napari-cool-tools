...
def ridge_analysis(
    ...
    if incedence_correction:
        ...
        match mode:
            case "fovea":
                ...
                if foveal_center[0] > -1 and foveal_center[1] > -1:
                    # if debug or verbose:
                    #     print(f"Using foveal center: {foveal_center}\n")
                    center_x = int(foveal_center[0])
                    center_y = int(foveal_center[1])
                # else:
        ...
        # x_left = np.linspace(-min_scan_angle,0,len(np.arange(x)[:center_x]))
        # x_right = np.linspace(0,min_scan_angle,len(np.arange(x)[center_x:]))
        # y_left = np.linspace(-min_scan_angle,0,len(np.arange(y)[:center_y]))
        # y_right = np.linspace(0,min_scan_angle,len(np.arange(y)[center_y:]))

        x_left = np.linspace(-scan_angle_from_center,0,len(np.arange(x)[:center_x]))
        x_right = np.linspace(0,scan_angle_from_center,len(np.arange(x)[center_x:]))
        y_left = np.linspace(-scan_angle_from_center,0,len(np.arange(y)[:center_y]))
        y_right = np.linspace(0,scan_angle_from_center,len(np.arange(y)[center_y:]))

        x_conv = np.concatenate([x_left,x_right],axis=0)
        y_conv = np.concatenate([y_left,y_right],axis=0)

        if debug:
            print(f"conversions: {x_conv},{y_conv}\n")
        ...
        #factor = np.cos(np.abs(x_rad))*np.cos(np.abs(y_rad))
        if debug:
            print(f"cos x_rad: {np.cos(x_rad)}\ncos y_rad: {np.cos(y_rad)}\n")
        factor = np.clip(factor,a_min=0.,a_max=1.)
        ...
        if corrected_thickness_vals.size == 0:
            print("This volume's ridge labels do not intersect with the retchor labels")
            #return -1.0,-1.0
            thickness_output["no_ridge_intersection_thickness"] = np.array([-1.0])
            thickness_output["micrometer_conversion_factor"] = 1.0
            return thickness_output
    ...
    if viewer:
        ...
        if incedence_correction:
            ...
            viewer.add_points(np.array([[center_x,center_y]]),size=4,face_color="green")
        if micrometer_output:
