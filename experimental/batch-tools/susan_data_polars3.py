...
def generate_seg_paper_stats(df,decimals:int=3):
    ...
    stats_out = pl.concat([stats,zone],how="horizontal").select(
        "Birth Weight (g)","Gestational Age (days)","Postmenstrual Age (days)","Zone","Average Zone","Infants","Eyes"
    )
    return stats_out
...
pre_plus = enface_project_out_df.filter(
    pl.col("Session Plus") == "Pre-Plus"
)
plus = enface_project_out_df.filter(
    pl.col("Session Plus") == "Plus"
)
stage_0 =  enface_project_out_df.filter(
    pl.col("Session Stage") == 0
)
stage_1 =  enface_project_out_df.filter(
    pl.col("Session Stage") == 1
)
stage_2 =  enface_project_out_df.filter(
    pl.col("Session Stage") == 2
)
stage_3 =  enface_project_out_df.filter(
    pl.col("Session Stage") == 3
)
