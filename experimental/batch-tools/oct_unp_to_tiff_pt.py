...
def dc_subtraction_double_sweep_pt(data):
    ...
    corrected_1 = corrected_1 - pt.tile(
        corrected_1.median(0)[0], (corrected_1.shape[0], 1)
    )  # pt.tile(pt.median(corrected_1,0),(corrected_1.shape[0],1))
    corrected_2 = corrected_2 - pt.tile(
        corrected_2.median(0)[0], (corrected_2.shape[0], 1)
    )  # pt.tile(pt.median(corrected_2,0),(corrected_2.shape[0],1))
...
def process_unp(
    ...
    if file_type == ".unp":
        ...
        with open(unp_file_path, "rb", buffering=0) as byte_reader:
            ...
            hamming = pt.tile(
                pt.hamming_window(subtracted_signal.shape[1]),
                (subtracted_signal.shape[0], 1),
            )  # hamming = np.tile(np.hamming(subtracted_signal.shape[1]),(subtracted_signal.shape[0],1))
            ...
            for frame_num in tqdm(
                ...
                # Subtract the DC signal
                subtracted_signal = dc_subtraction_double_sweep_pt(raw.squeeze())
