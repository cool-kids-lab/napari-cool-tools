...
def cartify_numpy (img_data:ImageData,spline_interpolation_order:int=3)->ImageData:
    ...
    if img_data.ndim >= 2:
        ...
        for _,b_scan in tqdm(enumerate(img_data),desc="processing slices"):
            ...
            image = normalize_data_in_range_func(b_scan,min_val=0.0,max_val=255.0).astype(np.uint8)
            ...
            height_pad = int(round(1.66*height*2)) #estimates for being in water and typical ref arm position
            ...
            image = np.pad(image, [(0,0),(height_pad,0)] , mode = 'constant', constant_values = 0)
            ...
            cart_image, ptSettings = polarTransform.convertToCartesianImage(image, initialAngle = -51*np.pi/180, finalAngle = 51*np.pi/180, hasColor = False, order=spline_interpolation_order)
...
def generate_fast_curve_correction(
    image_path: Path = Path(r"D:\JJ\Projects\Segmentation_Paper\Figures\Imaging_Data\b-scan_fold_4\imgs.prof"),
    label_path: Path = Path(r"D:\JJ\Projects\Segmentation_Paper\Figures\Imaging_Data\b-scan_fold_4\prs.prof"),
    output_dir: Path = Path(r"D:\JJ\Projects\Segmentation_Paper\Data\Bscan\Figure_Sample_Scans"),
    ...
    if preprocess_data:
        preproc_data = init_bscan_preproc(image_data,num_std=16,min_intensity=0.0,max_intensity=255.0,dtype=DTYPE.NP_UINT8)
    else:
    ...
    if preproc_data.dtype == np.float32:
        original_shape_data = resize(torch.Tensor(preproc_data),input_HW,interpolation=InterpolationMode.BILINEAR).numpy()
        original_label_shape_data = resize(torch.Tensor(label_data),input_HW,interpolation=InterpolationMode.NEAREST_EXACT).numpy()
    elif preproc_data.dtype == np.uint8:
        original_shape_data = resize(torch.Tensor(preproc_data),input_HW,interpolation=InterpolationMode.BILINEAR).numpy()
        original_label_shape_data = resize(torch.Tensor(label_data),input_HW,interpolation=InterpolationMode.NEAREST_EXACT).numpy()
    else:
        raise ValueError(f"Image data of dtype {preproc_data.dtype} is not supported, {np.float32},{np.uint8} are supported dtypes.")
    
    #output_data,settings = cartify_numpy(original_shape_data[370])
    #output_data = cartify_numpy(original_shape_data[370])
    output_data = cartify_numpy(original_shape_data[data_processing_range],spline_interpolation_order=3)
    label_output_data = cartify_numpy(original_label_shape_data[data_processing_range],spline_interpolation_order=0)

    #og_output_data = original_shape_data[data_processing_range].squeeze()
    og_output_data = normalize_data_in_range_func(original_shape_data[data_processing_range],min_val=0.0,max_val=1.0)
    og_output_data = og_output_data.squeeze()
    #og_label_output_data = original_label_shape_data[data_processing_range].squeeze()
    og_label_output_data = normalize_data_in_range_func(original_label_shape_data[data_processing_range],min_val=0.0,max_val=1.0)
    ...
    print(f"Original data shape: {original_shape_data.shape},{original_shape_data.dtype}")
    print(f"Output data shape: {output_data.shape},{output_data.dtype},{output_data.min()},{output_data.mean()},{output_data.max()}")
    print(f"Label output data shape: {label_output_data.shape},{label_output_data.dtype},{label_output_data.min()},{label_output_data.mean()},{label_output_data.max()}\n")
    #del viewer
    #gc.collect()
    print("og_output_data",type(og_output_data),og_output_data.shape,og_output_data.dtype,og_output_data.min(),og_output_data.mean(),og_output_data.max())
    print("og_label_output_data",type(og_label_output_data),og_label_output_data.shape,og_label_output_data.dtype,og_label_output_data.min(),og_label_output_data.mean(),og_label_output_data.max())

    retina_mask = ((label_output_data > retina_mask_range[0]) & (label_output_data < retina_mask_range[1]))
    #retina_mask = (output_data > 0.47) & (output_data < 0.472)
    choroid_mask = ((label_output_data >= choroid_mask_range[0]) & (label_output_data <= choroid_mask_range[1]))
    #choroid_mask = (output_data >= 0.472) & (output_data <= 1.0)

    color_retina = mask_to_color(label_output_data,retina_mask,ColorArray(retina_color))
    color_choroid = mask_to_color(label_output_data,choroid_mask,ColorArray(choroid_color))

    og_retina_mask = ((og_label_output_data > retina_mask_range[0]) & (og_label_output_data < retina_mask_range[1]))
    og_choroid_mask = ((og_label_output_data > choroid_mask_range[0]) & (og_label_output_data <= choroid_mask_range[1]))
    og_color_retina = mask_to_color(og_label_output_data,og_retina_mask,ColorArray(retina_color))
    og_color_choroid = mask_to_color(og_label_output_data,og_choroid_mask,ColorArray(choroid_color))
    ...
    if use_color_map:
        ...
        color_map = Colormap(colors=(color_0,color_1,color_2,color_3,color_4),controls=(0.0,0.47,0.471,0.472,1.0),interpolation='linear')
