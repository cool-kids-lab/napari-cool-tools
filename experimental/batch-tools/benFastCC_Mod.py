...
def cartify(data:np.ndarray, sweep=102, ds=1, res=1/6, threshold=5, chunk_size=16, save = False, circleCrop = 1):
...
def generate_fast_curve_correction(
    image_path: Path = Path(r"D:\JJ\Projects\Segmentation_Paper\Data\Bscan\Figure_Sample_Scans\fold_4_img_predictions.prof"),
    label_path: Path = Path(r"D:\JJ\Projects\Segmentation_Paper\Data\Bscan\Figure_Sample_Scans\fold_4_label_predictions.prof"),
    ...
    cart = cartify(data=preproc_data,sweep=sweep,ds=downsampling,res=resolution,threshold=threshold,save=save)
