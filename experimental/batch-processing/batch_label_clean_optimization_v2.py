    ...
    while True:
        ...
        for labels_path in label_progress:
            ...
            if not overwrite:
                ...
                existing_topology_present = False
                existing_topology_present2 = False
                existing_clean_ret_chor_present = False
            ...
            lens_labels_data,density_window_mask,lens_holder_mask = remove_lens_holder(
                test_labels_data,
                percentile=99,
                holder_height=64,
                pixel_offset=32,
            ...
            # window_start_idx = lens_holder_mask.nonzero()[1].min()
            # window_stop_idx = lens_holder_mask.nonzero()[1].max()
            # density_window_mask = np.zeros_like(test_labels_data,dtype=np.uint8)
            # density_window_mask[:,window_start_idx:window_stop_idx,:] = 9
            ...
            for label in label_pbar:
                ...
                overhanging_labels = (desired_labels==1) & (overhang_artefact_projection > 0) & (density_window_mask > 0)
                overhanging_labels = overhanging_labels.astype("uint8")
                #overhanging_labels = torch.logical_and(overhanging_labels,density_window_mask)
                ...
                if overwrite | (save_topological_map and not (existing_topology_present and existing_topology_present2)):
            ...
            if overwrite | (save_clean_labels and not existing_clean_ret_chor_present):
            ...
            if viewer:
                ...
                viewer.add_labels(density_window_mask,name=f"{id}_cowl_mask2")
                ...
                viewer.add_labels(lens_holder_mask*4,name=f"{id}_lens_holder_mask",visible=False)
