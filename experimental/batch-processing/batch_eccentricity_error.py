...
def main():
    ...
    output_dir = Path(r"\\192.168.1.3\coolkid\Beth Roti\Ridge_Height_Troubelshoot\Test_Eccentricity_Error")
    ...
    retchor_path_generator = output_dir.glob("*_ret_chor_seg_clean.npz")
    ...
    for retchor_path in retchor_progress_bar:
        ...
        for label_val in label_pbar:
            ...
            if label_val == 2:
                if verbose:
                    print("Choroid does not need to be calculated here...Skipping\n")
                continue
            ...
            #     "id_number": id_number,
            #     "tissue": tissue_map[label_val-1],
            #     "radii": radius_calc*pixel_spacing,
            #     "min_thickness": curve_correct_height.min(),
            #     "mean_thickness": curve_correct_height.mean(),
            #     "max_thickness": curve_correct_height.max(),
            # })
            ...
            # # Perform profile calculations and save

            if perform_calculations:

                raw_micron_thickness_map = raw_pixel_thickness_map*conv_factor
                pixel_difference_map = np.round(raw_pixel_thickness_map).astype("uint16") - np.round(pixel_thickness_map).astype("uint16")
                ...
                sampled_pixels = equidistant_pixel_error_by_degree(micron_error_map,scan_angle=cc_settings.scan_angle)
    ...
    if perform_calculations and (save_plots or show_plots):
        ...
        target_angle = shapes_arr.max()

        aligned_eccentricity_values = [arr for arr in eccentricity_error_values if arr.shape[0] == target_angle]
    ...
    if output_dataframe is not None and not output_dataframe.is_empty():
    # if len(radius_list) > 0:
        ...
        radii_df = output_dataframe.filter((pl.col("radii") < 24) & (pl.col("radii") > 5.5))
        print(output_dataframe)
        print(radii_df)
        radii_ndarray = np.array(radii_df["radii"].to_list())
