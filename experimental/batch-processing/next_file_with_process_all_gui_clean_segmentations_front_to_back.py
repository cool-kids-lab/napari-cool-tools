...
from pytorch_unp_batch_processing import save_npz
...
def process_file(file_path:Path,masks:tuple,logger=''):
    ...
    if file_path.suffix == ".npz":
        layer_data = npz_file_reader(file_path)[0]
        label_data,_,_ = layer_data
    ...
    if label_out_data.sum() == 0:
        exit_code = 1
    else:
        save_dict = {"name":f"{file_path.stem}_clean","layer_type":"labels","shape":label_out_data.shape}
        save_npz(path=save_file_path,data=label_out_data,save_dict=save_dict)
        exit_code = 0
        ...
        try:
            ...
            #for file_path in tqdm(reversed(self.file_list),desc="Processing_Files"):
                ...
                #print(save_file_path)
                    ...
                    #if not save_file_path.exists(): #file_path.with_suffix(".npz").exists():
