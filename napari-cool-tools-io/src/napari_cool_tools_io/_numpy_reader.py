...
def numpy_get_reader(path):
    """Reader for COOL lab .npy file format.
    ...
    if isinstance(path, str) and path.endswith(".npy"):
        ...
        return numpy_file_reader
    return None


def numpy_file_reader(path):
    ...
    file_name = Path(path).stem  # attributes["name"]
