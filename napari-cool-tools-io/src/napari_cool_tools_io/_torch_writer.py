    ...
    if len(layer_data) == 1:
        ...
        for layer in layer_data:
            ...
            if data.ndim == 3:
                # reverse flip and transpose that occured upon loading
                save_data = flip(data, 1).transpose(0, 2, 1)
                worker = ndarray_to_file_thread(
                    out_path, save_data, attributes, layer_type
                )
                worker.start()
                # save_data.tofile(path)
...
def torch_save_thread(data: any, path: Path):
    """"""
    show_info("torch save thread started!!\n")
    torch_data = torch.save(data, path, pickle_protocol=HIGHEST_PROTOCOL)
    show_info("torch save thread completed!!\n")
    return torch_data
...
@magic_factory(
    ...
    key={
        "label": "key",
        "visible": False,
        "tooltip": "Saves data in dictionary with key",
    },
    file_name={
        "label": "file_name",
        "visible": False,
        "tooltip": "File name to save image-label data",
    },
    img_key={
        "label": "img_key",
        "visible": False,
        "tooltip": "Saves image data in dictionary with key",
    },
    lbl_key={
        "label": "lbl_key",
        "visible": False,
        "tooltip": "Saves label data in dictionary with key",
    },
    img={"label": "img", "visible": False},
    lbls={"label": "lbls", "visible": False},
    call_button="Export .pt file",
)
def torch_file_exporter(
    out_type: OutputType = OutputType.Data,
    path: Path = Path("./"),
    key: str = "",
    file_name: str = "image-labels",
    img_key: str = "images",
    lbl_key: str = "labels",
    img: Layer = None,
    lbls: Layer = None,
...
def torch_file_importer(torch_file: Path) -> List[Layer]:
    ...
    if type(torch_data) is torch.Tensor:
        show_info(
            f"Pytorch data contains a tensor.\n\nLoading data...\n\n{torch_data}\n"
        )
        ...
        return [layer]
    elif type(torch_data) is dict:
        show_info(
            f"Pytorch data contains a dictionary.\n\nLoading data...\n\n{torch_data}\n"
        )
        out_images = []
        for key, val in torch_data.items():
            if type(val) is torch.Tensor:
                val = val.numpy()
                show_info(
                    f"Dictionary contains key('{key}') with value of type {type(val)}.\n"
                )
    ...
    # else display contents via show_info
        ...
        show_info(
            f"Pytorch data is not a tensor or a dictionary.\n\nIt contains a {type(torch_data)}:\n\n{torch_data}\n"
        )
        return [None]
